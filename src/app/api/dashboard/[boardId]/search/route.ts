...
export async function GET(
  ...
  try {
    ...
    if (session?.user) {
      ...
      const pattern = `${query}%`;
      const sectionsSql = `  SELECT bs.id, s.name, s.url, IF(sc.badge_count IS NULL, 0, sc.badge_count) AS badge_count
        ...
        WHERE bs.id_board = ? AND bs.archived IS NULL AND s.name LIKE ?`;
      const sectionsValues = [id, boardId, pattern];
