...
export default function AddPDFPage() {
  ...
  async function handleSubmitPDF(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    ...
    if (!name) {
      setLoading(false);
      return setSnackbar("Chybí název PDF!");
    }

    const formData = new FormData(e.currentTarget);

    const pdf = formData.get("pdf");
    if (pdf instanceof Blob && !pdf.size) {
      setLoading(false);
      return setSnackbar("Nahrajte PDF!");
    }

    formData.append("name", name);

    const res = await fetch(
      `/api/dashboard/${params.boardId}/${params.sectionId}/pdf`,
      {
        method: "POST",
        body: formData,
      }
  ...
  return (
    <>
      ...
      <StepLayout step={step + 1} maxStep={2}>
        {step === 0 && (
          <>
            <section className="flex-1 flex flex-col justify-center items-center">
              <h2 className="text-headline-medium mb-6">Zadejte název PDF</h2>
            ...
            <div className="mt-6 w-full flex justify-between gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (!name) {
                    return setNameError("Zadejte název PDF.");
                  } else {
                    if (!nameError) {
                      setStep(1);
                    }
        ...
        {step === 1 && (
          <form onSubmit={handleSubmitPDF} className="flex-1 flex flex-col">
            <section className="flex-1 flex flex-col justify-start items-center">
              <h2 className="text-headline-medium mb-6">Nahrajte PDF</h2>
