...
export default function AddFilePage() {
  ...
  async function handleSubmitFile(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();

    if (!name) {
      return setSnackbar("Chybí název souboru!");
    }
    ...
    if (file instanceof File && !file.size) {
      return setSnackbar("Nahrajte soubor!");
    }

    formData.append("name", name);

    const res = await fetch(
      `/api/dashboard/${params.boardId}/${params.sectionId}/file`,
      {
        method: "POST",
        body: formData,
      }
  ...
  return (
    <>
      ...
      <StepLayout>
          ...
          <>
            ...
            <div className="mt-6 w-full flex justify-end">
              <Button
                ...
                onClick={() => {
                  if (!name) {
                    return setNameError("Zadejte název souboru.");
                  } else {
                    if (!nameError) {
                      setStep(1);
                    }
        ...
        {step === 1 && (
          <form onSubmit={handleSubmitFile} className="flex-1 flex flex-col">
            <section className="flex-1 flex flex-col justify-start items-center">
              <h2 className="text-headline-medium mb-6">Nahrajte soubor</h2>
